...
n=$(cat config.json |python -c "import json,sys;print len(json.load(sys.stdin)['projects'])")
...
for i in $(seq 1 $n);do
  name=$(cat config.json |python -c "import json,sys;print json.load(sys.stdin)['projects'][${i}-1]['name']")
  hostname=$(cat config.json |python -c "import json,sys;print json.load(sys.stdin)['projects'][${i}-1]['hostname']")
  generic_hostname=$(cat config.json |python -c "import json,sys;print json.load(sys.stdin)['projects'][${i}-1]['generic-hostname']")
  tls_enabled=$(cat config.json |python -c "import json,sys;print json.load(sys.stdin)['projects'][${i}-1]['tls-enabled']")
